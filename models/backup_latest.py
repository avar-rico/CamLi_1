...
class PWCFusionCore(nn.Module):
    ...
    def __init__(self, cfgs2d, cfgs3d, debug=False):
        ...
        self.flow_estimator_2d = FlowEstimator2D(
            [64 + corr_channels_2d + 2 + 32, 128, 64, 32],
            norm=cfgs2d.norm.flow_estimator,
            conv_last=False,
        )
        ...
        self.flow_upsample_2d = torch.nn.ConvTranspose2d(
                                    in_channels=2, out_channels=2, kernel_size=4, stride=2, padding=1, bias=False,
                                    groups=2)
    ...
    def decode(self, xyzs1, xyzs2, feats1_2d, feats2_2d, feats1_3d, feats2_3d, camera_info):
        ...
        for level in range(len(feats1_2d) - 1, 0, -1):
            ...
            if level == len(feats1_2d) - 1:
                ...
                last_flow_2d = self.flow_upsample_2d(flows_2d[-1])
                ...
                x_2d = torch.cat([feat_corr_2d, feat1_2d, last_flow_2d, last_flow_feat_2d], dim=1)
                flow_feat_2d = self.flow_estimator_2d(x_2d)  # [bs, 32, image_h, image_w]
