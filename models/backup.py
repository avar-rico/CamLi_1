...
class PWCFusionCore(nn.Module):
    ...
    def __init__(self, cfgs2d, cfgs3d, debug=False):
        ...
        # 不用densenet,channel太多了
        # self.flow_estimator_2d = FlowEstimator2D(
        #     [64 + corr_channels_2d + 2 + 32, 128, 128, 96],
        ...
        # interpolation for 3D points
        # self.point_interpolate = Geometric_knn_interpolation()
    ...
    def decode(self, xyzs1, xyzs2, feats1_2d, feats2_2d, feats1_3d, feats2_3d, camera_info):
        ...
        for level in range(len(xyzs1) - 1, 0, -1):
            ...
            # 这里的leaky_relu表示corr_2d对负值进行了缩小
            ...
            # align features using 1x1 convolution， 降低维度
        ...
        # 为什么剩下的flow都upscale 4倍
        for i in range(1, len(flows_2d)):
            # b, _, h, w = flows_2d[i].shape
            # cur_flow = flows_2d[i].reshape([b, 2, -1])
            # proj_xy = project_pc2image(xyzs1[i-1], camera_info)
            # cur_xy = proj_xy.detach()
            # sensor_h, sensor_w = camera_info['sensor_h'], camera_info['sensor_w']
            # cur_xy[:, 0] *= (w - 1) / (sensor_w - 1)
            # cur_xy[:, 1] *= (h - 1) / (sensor_h - 1)
            # cur_grid = mesh_grid(b, h, w, cur_flow.device)
            # cur_grid = cur_grid.reshape([b, 2, -1])
            # tmp_flow_2d = self.point_interpolate(cur_grid, cur_flow, cur_xy, ignoreFirst=False) * 4
            # new_xy = proj_xy.detach()
            # new_xy[:, 0] *= (4*w - 1) / (sensor_w - 1)
            # new_xy[:, 1] *= (4*h - 1) / (sensor_h - 1)
            # new_grid = mesh_grid(b, 4*h, 4*w, cur_flow.device)
            # new_grid = new_grid.reshape([b, 2, -1])
            # flows_2d[i] = self.point_interpolate(new_xy, tmp_flow_2d, new_grid, ignoreFirst=False).reshape(b, 2, 4*h, 4*w)
        ...
        for i in range(len(flows_3d)):
            ...
            # flows_3d[i] = self.point_interpolate(xyzs1[i + 1], flows_3d[i], xyzs1[i])
