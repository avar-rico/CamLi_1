...
class Geometric_knn_interpolation(nn.Module):
    def __init__(self, k=3):
        super().__init__()
        self.k = k
        self.fc = nn.Linear(k**2, k)

    def forward(self, input_xyz, input_features, query_xyz, ignoreFirst=True):
        # input_xyz, query_xyz = input_xyz.detach(), query_xyz.detach()

        if ignoreFirst:
            knn_indices = k_nearest_neighbor(input_xyz, query_xyz, self.k + 1)[:, :, 1:]  # [batch_size, n_queries, k]
        else:
            knn_indices = k_nearest_neighbor(input_xyz, query_xyz, self.k)
        knn_xyz = batch_indexing_channel_first(input_xyz, knn_indices)  # [batch_size, 3, n_queries, k]

        knn_vector = knn_xyz - query_xyz[..., None]
        knn_tensor = torch.einsum('b d n k, b d n q-> b n k q', knn_vector, knn_vector)
        batch, n_queries, _, k = knn_tensor.shape
        dens = [knn_tensor[:, :, i, i].unsqueeze(2) for i in range(k)]
        dens = torch.cat(dens, dim=2)
        knn_tensor = knn_tensor / dens[:, :, :, None]
        knn_tensor = knn_tensor.reshape(batch, n_queries, -1)
        knn_weights = torch.tanh(self.fc(knn_tensor))
        knn_weights = knn_weights / torch.sqrt(torch.sum(knn_weights**2, dim=-1, keepdim=True))  # [bs, n_queries, k]
...
def knn_interpolation(input_xyz, input_features, query_xyz, k=3, ignoreFirst=False, isGemotric=False):
    """
    ...
    """
    # 从input_xyz里给query_xyz里的每个点找k个最近点，用对应距离的倒数作为权重进行插值
    if ignoreFirst:
        knn_indices = k_nearest_neighbor(input_xyz, query_xyz, k+1)[:, :, 1:]  # [batch_size, n_queries, k]
    else:
    ...
    if isGemotric:
        ...
        knn_tensor = torch.einsum('b d n k, b d n q-> b n k q', knn_vector, knn_vector)
        batch, n_queries, _, k = knn_tensor.shape
        a = torch.ones([batch, n_queries, 1, k], device=knn_tensor.device)
        A = torch.cat([knn_tensor, a], dim=2)
        b = torch.Tensor([[0], [0], [0], [1]]).cuda()
        b = b[None, None, :, :].repeat(batch, n_queries, 1, 1)
        AA = torch.matmul(A.permute(0, 1, 3, 2), A)
        Ab = torch.matmul(A.permute(0, 1, 3, 2), b)
        # AA = torch.einsum('b n q k, b n k v-> b n k v', A, A)
        # Ab = torch.einsum('b n q k, b n k s-> b n q s', A, b)
        invs = []
        for i in range(batch):
            for j in range(n_queries):
                invs.append(torch.inverse(AA[i, j, :, :]))
        AA_inverse = torch.cat(invs, dim=0).reshape(batch, n_queries, k, k)
        knn_weights = torch.einsum('b n k q, b n q s->b n k s', AA_inverse, Ab).squeeze()
        # knn_weights = torch.einsum('b n k q, b n q s->b n k s', torch.inverse(AA), Ab)
    else:
        knn_vector = knn_xyz - query_xyz[..., None] # [batch_size, 3, n_queries, k]
        # 用这个算，只有正的，没负的项
        knn_dists = torch.linalg.norm(knn_vector, dim=1).clamp(1e-8)  # [bs, n_queries, k]
...
def project_feat_with_nn_corr(xy, feat_2d, feat_3d, nn_indices=None):
    ...
    # 这里绕了一圈，先从2d的feat map插值得到3d投影点的feat map,再得到3d的投影点离网格最近的点的feat map，又回到2d
    # 用2d feat绕一圈得到的每个网格点的特征
    ...
    # 离2d网格最近的3d到2d的投影点的坐标减去对应的网格坐标
    nn_offset = batch_indexing_channel_first(xy, nn_indices) - grid  # [B, 2, HW]
    # 对应channel位置逐元素相乘，再取平均（网格点与最近的投影点的相似性，但是投影点的特征本来就是插值得到的，这个相似性感觉冗余了）
