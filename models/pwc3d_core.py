...
def build_pc_pyramid(pc1, pc2, n_samples_list):
    ...
    # select and resort the points using fps, then only slicing is required in the subsequent operations
...
class FeaturePyramid3D(nn.Module):
    def __init__(self, n_channels, norm=None, k=16):
        ...
        self.level0_mlp = MLP1d(3, [n_channels[0]])
        self.level0_pointconv = PointConvNoSampling(n_channels[0], n_channels[0], norm=norm, k=k)
        ...
        for i in range(len(n_channels) - 1):
            if i == 0:
                self.pyramid_mlps.append(MLP1d(n_channels[i], [n_channels[i + 1]]))
            else:
    ...
    def forward(self, xyzs):
        ...
        # 这个干嘛用的,提供一个8192的feat?
        inputs = torch.zeros_like(xyzs[0])  # [bs, 3, n_points]
        tmp_feats = self.level0_mlp(inputs)  # [bs, 32, n_points]
        feats = [self.level0_pointconv(xyzs[0], tmp_feats)]  # [bs, 32, n_points]
...
#         # new added to constrain the value to range [-1, 1]
...
class FlowEstimator3D(nn.Module):
    def __init__(self, n_channels, norm=None, conv_last=True, k=16):
        super().__init__()
        self.point_conv1 = PointConvNoSampling(in_channels=n_channels[0], out_channels=n_channels[1], norm=norm, k=k)
        self.mlp = MLP1d(n_channels[1], [n_channels[2]])

        if conv_last:
            self.conv_last = nn.Conv1d(n_channels[2], 3, kernel_size=1)
        else:
            self.conv_last = None
