...
from .utils import MLP1d, MLP2d, batch_indexing_channel_first, batch_indexing_channel_last
from .csrc import k_nearest_neighbor


class WeightNet(MLP2d):
    def __init__(self, in_channels, mlps, norm=None, activation='leaky_relu'):
        super(WeightNet, self).__init__(in_channels, mlps, norm, activation)
...
class PointConvDownSampling(nn.Module):
    def __init__(self, in_channels, out_channels, norm=None, activation='leaky_relu', k=16):
        ...
        self.weight_net = WeightNet(3, [8, 16], activation=activation)
