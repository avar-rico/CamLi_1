...
class Trainer:
    ...
    def train_one_epoch(self):
        ...
        for i, inputs in enumerate(self.train_loader):
            ...
            pos1, pos2 = inputs['pcs'][:, :3, :], inputs['pcs'][:, 3:, :]
            pos1 = pos1[1].transpose(0, 1).cpu().numpy()
            pos2 = pos2[1].transpose(0, 1).cpu().numpy()
            display_sub(pos1, pos2)
    ...
    def validate(self):
        ...
        for i, inputs in enumerate(self.val_loader):
            ...
            if (i + 1) % self.cfgs.log.save_summary_every_n_steps == 0:
...
def display_sub(pc1, pc2):
    ...
    point_cloud1.colors = open3d.utility.Vector3dVector(np.zeros_like(pc1) + [0, 0, 1])
    point_cloud2.colors = open3d.utility.Vector3dVector(np.zeros_like(pc2) + [1, 0, 0])
    open3d.visualization.draw_geometries([point_cloud1, point_cloud2])


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--config', required=False, default='conf/train/pretrain.yaml',
