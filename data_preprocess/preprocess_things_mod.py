...
parser.add_argument('--input_dir', required=False, default=r'D:\CV-dataset-backup\FlyingThings3D_subset', help='Path to the FlyingThings3D subset')
parser.add_argument('--output_dir', required=False, default=r'G:\CV-dataset\OpticalFlow\Flyingthings3D_subset\new_processed')
parser.add_argument('--n_points', required=False, default=[32768, 8192])
parser.add_argument('--max_depth', required=False, default=35.0)
parser.add_argument('--remove_occluded_points', default=True, action='store_true')
...
class Preprocessor(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        occ_disp = cv2.imread(os.path.join(
            self.input_dir, self.split, 'disparity_occlusions', 'left', '%07d.png' % index1
        ))
        ...
        valid_mask = np.logical_and(occ_disp[..., 0] == 0, occ_mask_2d[..., 0] == 0)
        ...
        # random sampling, shuffle the order of 3D points
